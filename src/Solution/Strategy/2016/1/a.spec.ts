...
describe('Solution20161', () => {
    describe('solve', () => {
        it('interprets R2, L3 as 2', async () => {
            ...
            const solution = await new Solution20161a(stringArr).solve();
    ...
    describe('cardinalDirectionToAxis', () => {
        it('interprets North as NorthSouth', async () => {
            expect(
                Solution20161a.cardinalDirectionToAxis(CardinalDirection.North)
            ).to.equal(Axis.NorthSouth);
        });
        it('interprets East as EastWest', async () => {
            expect(
                Solution20161a.cardinalDirectionToAxis(CardinalDirection.East)
            ).to.equal(Axis.EastWest);
        });
        it('interprets South as NorthSouth', async () => {
            expect(
                Solution20161a.cardinalDirectionToAxis(CardinalDirection.South)
            ).to.equal(Axis.NorthSouth);
        });
        it('interprets West as EastWest', async () => {
            expect(
                Solution20161a.cardinalDirectionToAxis(CardinalDirection.West)
            ).to.equal(Axis.EastWest);
        });
    });
    describe('cardinalDirectionToFactor', () => {
        it('interprets North as NorthEast', async () => {
            expect(
                Solution20161a.cardinalDirectionToFactor(
        ...
        it('interprets East as NorthEast', async () => {
            expect(
                Solution20161a.cardinalDirectionToFactor(CardinalDirection.East)
            ).to.equal(Factor.NorthEast);
        });
        ...
        it('interprets West as SouthWest', async () => {
            expect(
                Solution20161a.cardinalDirectionToFactor(CardinalDirection.West)
            ).to.equal(Factor.SouthWest);
        });
    });
    describe('rotate', () => {
    ...
    describe('parseInstruction', () => {
        it('interprets R23 as Right and 23', async () => {
            expect(Solution20161a.parseInstruction('R23')).to.deep.equal({
                handDirection: HandDirection.R,
                movement: 23,
            });
        });
        it('interprets L101 as Left and 101', async () => {
            expect(Solution20161a.parseInstruction('L101')).to.deep.equal({
                handDirection: HandDirection.L,
                movement: 101,
            });
        });
        it('fails to parse ABC', async () => {
            expect(() => Solution20161a.parseInstruction('ABC')).to.throw(
                `Unable to parseInstruction: ABC`
            );
        });
        it('fails to parse Z12', async () => {
            expect(() => Solution20161a.parseInstruction('Z12')).to.throw(
