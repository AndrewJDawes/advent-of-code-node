...
describe('InputFetcher Default factory', () => {
    it('returns service with getIterator method when given a url type', () => {
        const factory = new Default();
        const service = factory.getService('url', 'https://example.com');
        expect(service['getIterator']).to.be.a('function');
    });
    it('returns service with getIterator method when given a file type', () => {
        const factory = new Default();
        const service = factory.getService('file', 'https://example.com');
        expect(service['getIterator']).to.be.a('function');
    });
    it('returns service with getIterator method when given an array type', () => {
        const factory = new Default();
